...
<div class="min-h-screen bg-gray-100">
    ...
    <div class="container mx-auto p-6">
        <h1 class="text-3xl font-bold mb-6">Créer une nouvelle adresse</h1>
        ...
        <form action="{{ route('user.addresses.store') }}" method="POST" class="bg-white p-6 rounded shadow-md">
            @csrf
            <div class="mb-4">
                <label class="block font-bold mb-1">Latitude</label>
                <input type="text" name="latitude" class="border p-2 w-full rounded" value="{{ old('latitude') }}" required>
            </div>
            <div class="mb-4">
                <label class="block font-bold mb-1">Longitude</label>
                <input type="text" name="longitude" class="border p-2 w-full rounded" value="{{ old('longitude') }}" required>
            </div>
            <div class="mb-4">
                <label class="block font-bold mb-1">Description</label>
                <textarea name="description" class="border p-2 w-full rounded" required>{{ old('description') }}</textarea>
            </div>
            <div class="mb-4">
                <label class="block font-bold mb-1">Repère local</label>
                <input type="text" name="repere_local" class="border p-2 w-full rounded" value="{{ old('repere_local') }}" required>
            </div>
            <div class="flex items-center space-x-4">
